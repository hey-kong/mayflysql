...
from mayflysql.core.database import Database
from mayflysql.core import SerializedInterface
from mayflysql.parser import SQLParser
...
class Engine:
    ...
    def search(self, table_name, fields='*', sort='ASC', **conditions):
        ...
        return self.__get_table(table_name).search(
            fields=fields, sort=sort, format_type=self.__format_type, **conditions)
    ...
    def __check_is_choose(self):
        ...
        if not self.__current_db or not isinstance(
                self.__current_db, Database):
    ...
    def execute(self, statement):
        ...
        if action['type'] in self.__action_map:
            ...
            if action['type'] in ['insert', 'update',
                                  'delete', 'create', 'drop']:
                self.commit()
    ...
    def run(self):
        while True:
            # 获得输入的 SQL 语句
            statement = input('\033[00;37mmayflysql> ')
            try:
